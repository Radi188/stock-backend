...
interface StoreConfig {
  ...
  purchaseInvoices: PurchaseInvoiceDef[];
  orders: OrderDef[];
}
...
interface UserDef {
  ...
  password: string;
  role: UserRole;
  /** store names this user has access to */
  storeNames: string[];
}
...
const stores: StoreConfig[] = [
  ...
  // ═══════════════════════════════════════════════════════════════
  {
    ...
    orders: [
      { tableNumber: 'T-01', customerName: 'Walk-in', status: OrderStatus.DELIVERED, items: [{ name: 'Latte', quantity: 2 }, { name: 'Butter Croissant', quantity: 2 }] },
      { tableNumber: 'T-03', customerName: 'Walk-in', status: OrderStatus.DELIVERED, items: [{ name: 'Cappuccino', quantity: 1 }, { name: 'Club Sandwich', quantity: 1 }] },
      { tableNumber: 'T-05', customerName: 'Walk-in', status: OrderStatus.DELIVERED, items: [{ name: 'Cold Brew', quantity: 2 }, { name: 'Avocado Toast', quantity: 2 }] },
      { tableNumber: 'T-07', customerName: 'Walk-in', status: OrderStatus.READY, items: [{ name: 'Frappuccino', quantity: 3 }, { name: 'Blueberry Muffin', quantity: 2 }] },
      { tableNumber: 'T-02', customerName: 'Walk-in', status: OrderStatus.PREPARING, items: [{ name: 'Espresso', quantity: 2 }, { name: 'Cinnamon Roll', quantity: 1 }] },
      { tableNumber: 'T-06', customerName: 'Walk-in', status: OrderStatus.CONFIRMED, items: [{ name: 'Iced Latte', quantity: 2 }, { name: 'Mango Smoothie', quantity: 1 }] },
      { tableNumber: 'T-08', customerName: 'Walk-in', status: OrderStatus.PENDING, items: [{ name: 'Thai Milk Tea', quantity: 2 }, { name: 'Matcha Latte', quantity: 1 }] },
      { tableNumber: 'T-04', customerName: 'Walk-in', status: OrderStatus.PENDING, items: [{ name: 'Flat White', quantity: 1 }, { name: 'Banana Bread Slice', quantity: 1 }] },
    ],
  },
  ...
  // ═══════════════════════════════════════════════════════════════
  {
    ...
    orders: [
      { customerName: 'Sophia N.', status: OrderStatus.DELIVERED, items: [{ name: 'Hyaluronic Acid Serum', quantity: 1 }, { name: 'Mineral Sunscreen SPF 50+', quantity: 2 }] },
      { customerName: 'Emma L.', status: OrderStatus.DELIVERED, items: [{ name: 'Gentle Foam Cleanser', quantity: 2 }, { name: 'Niacinamide 10% Serum', quantity: 1 }] },
      { customerName: 'Nina R.', status: OrderStatus.DELIVERED, items: [{ name: 'Vitamin C Brightening Serum', quantity: 1 }, { name: 'Ceramide Barrier Cream', quantity: 1 }] },
      { customerName: 'Amy K.', status: OrderStatus.CONFIRMED, items: [{ name: 'AHA/BHA Exfoliating Toner', quantity: 1 }, { name: 'Oil Cleansing Balm', quantity: 1 }] },
      { customerName: 'Grace P.', status: OrderStatus.PENDING, items: [{ name: 'Retinol Night Serum', quantity: 1 }, { name: 'Tinted Sunscreen SPF 40', quantity: 1 }] },
      { customerName: 'Iris T.', status: OrderStatus.PENDING, items: [{ name: 'Micellar Cleansing Water', quantity: 2 }] },
    ],
  },
  ...
  // ═══════════════════════════════════════════════════════════════
  {
    ...
    orders: [
      { customerName: 'Alex H.', status: OrderStatus.DELIVERED, items: [{ name: 'iPhone 15 Pro 256GB', quantity: 1 }, { name: 'AirPods Pro 2nd Gen', quantity: 1 }] },
      { customerName: 'Ben T.', status: OrderStatus.DELIVERED, items: [{ name: 'Samsung Galaxy S24 256GB', quantity: 2 }] },
      { customerName: 'Chris W.', status: OrderStatus.DELIVERED, items: [{ name: 'MacBook Air M3 15"', quantity: 1 }] },
      { customerName: 'Diana F.', status: OrderStatus.PREPARING, items: [{ name: 'Sony WH-1000XM5', quantity: 1 }, { name: 'JBL Charge 5', quantity: 1 }] },
      { customerName: 'Eddie S.', status: OrderStatus.CONFIRMED, items: [{ name: 'ASUS ROG Strix G16', quantity: 1 }] },
      { customerName: 'Fiona K.', status: OrderStatus.PENDING, items: [{ name: 'Xiaomi 14 256GB', quantity: 1 }, { name: 'Samsung Galaxy Buds3 Pro', quantity: 1 }] },
    ],
  },
  ...
  // ═══════════════════════════════════════════════════════════════
  {
    ...
    orders: [
      { customerName: 'ACE Contractors', status: OrderStatus.DELIVERED, items: [{ name: 'Portland Cement 50kg', quantity: 50 }, { name: 'Steel Rebar 12mm x 10m', quantity: 30 }, { name: 'River Sand 1 Ton', quantity: 5 }] },
      { customerName: 'Prime Builders', status: OrderStatus.DELIVERED, items: [{ name: 'Concrete Hollow Block 20cm', quantity: 200 }, { name: 'Portland Cement 50kg', quantity: 20 }] },
      { customerName: 'Solo Contractor', status: OrderStatus.DELIVERED, items: [{ name: 'Angle Grinder 4.5"', quantity: 2 }, { name: 'Corded Drill 13mm', quantity: 1 }] },
      { customerName: 'Mega Build Co.', status: OrderStatus.CONFIRMED, items: [{ name: 'Portland Cement 50kg', quantity: 100 }, { name: 'Steel Rebar 16mm x 10m', quantity: 50 }] },
      { customerName: 'Home Renovator', status: OrderStatus.CONFIRMED, items: [{ name: 'PVC Pipe 4" x 4m', quantity: 10 }, { name: 'Ball Valve 1"', quantity: 5 }] },
      { customerName: 'Electrician Joe', status: OrderStatus.PENDING, items: [{ name: 'THW Cable 2.5mm 100m', quantity: 5 }, { name: 'Circuit Breaker 20A', quantity: 10 }] },
    ],
  },
  ...
  // ═══════════════════════════════════════════════════════════════
  {
    ...
    orders: [
      { customerName: 'Regular Customer', status: OrderStatus.DELIVERED, items: [{ name: 'Coca-Cola 1.5L', quantity: 4 }, { name: "Lay's Original 75g", quantity: 3 }, { name: 'KitKat 4-Finger', quantity: 5 }] },
      { customerName: 'Family Shop', status: OrderStatus.DELIVERED, items: [{ name: 'Full Cream Milk 1L', quantity: 3 }, { name: 'Cheddar Cheese 250g', quantity: 2 }, { name: 'Butter 250g', quantity: 1 }] },
      { customerName: 'Weekly Shopper', status: OrderStatus.DELIVERED, items: [{ name: 'Ariel Laundry Powder 3kg', quantity: 2 }, { name: 'Fairy Dishwashing Liquid 500ml', quantity: 2 }, { name: 'Mineral Water 1.5L', quantity: 6 }] },
      { customerName: 'Office Order', status: OrderStatus.CONFIRMED, items: [{ name: 'Mineral Water 600ml', quantity: 24 }, { name: 'Red Bull 250ml', quantity: 12 }, { name: 'Milo 3in1 Pack 20s', quantity: 4 }] },
      { customerName: 'Party Supplies', status: OrderStatus.CONFIRMED, items: [{ name: 'Pringles Original 165g', quantity: 5 }, { name: 'Oreo Original 154g', quantity: 4 }, { name: 'Coca-Cola 1.5L', quantity: 6 }] },
      { customerName: 'Quick Grab', status: OrderStatus.PENDING, items: [{ name: 'Snickers 50g', quantity: 3 }, { name: 'Red Bull 250ml', quantity: 2 }] },
    ],
  },
];

// ─── Users (defined globally — each can belong to multiple stores) ──────────
...
async function main() {
  ...
  const piService = app.get(PurchaseInvoicesService);
  const ordersService = app.get(OrdersService);

  const storeRepo = ds.getRepository(Store);
  const userRepo = ds.getRepository(User);
  const categoryRepo = ds.getRepository(Category);
  const subcategoryRepo = ds.getRepository(Subcategory);
  ...
  // ── 2. Create all stores first (so users can reference them) ──
  ...
  // ── 3. Create users with multi-store assignments ───────────────
  ...
  for (const u of seedUsers) {
    ...
    const user = await userRepo.save(
      userRepo.create({ ...u, password: await hash(u.password), stores: assignedStores }),
    );
  ...
  // ── 4. Seed categories, items, suppliers, invoices, orders ────
  for (const config of stores) {
    const store = storeMap.get(config.store.name as string)!;
    const storeUsers = usersByStore.get(store.name) ?? [];
    console.log(`🏪  ${store.name}  (${storeUsers.length} users)`);

    // Categories → Subcategories → MenuItems
    let totalItems = 0;
    for (const catDef of config.categories) {
      const category = await categoryRepo.save(
        categoryRepo.create({ name: catDef.name, store_id: store.id, sortOrder: catDef.sortOrder }),
      );
      for (const subDef of catDef.subcategories) {
        const sub = await subcategoryRepo.save(
          subcategoryRepo.create({ name: subDef.name, category_id: category.id, sortOrder: subDef.sortOrder }),
        );
    ...
    for (const sDef of config.suppliers) {
      supplierMap[sDef.name] = await supplierRepo.save(
        supplierRepo.create({ ...sDef, store_id: store.id }),
      );
    }

    // Purchase invoices → auto-adds inventory batches on receive
    const menuItems = await menuItemRepo.find({ where: { store_id: store.id } });
    const itemByName = (n: string) => {
      const found = menuItems.find((m) => m.name === n);
      if (!found) throw new Error(`Item not found: "${n}" in ${store.name}`);
      return found;
    };
    ...
    // Orders → CONFIRMED orders auto-deduct stock via FIFO
    ...
    for (const orderDef of config.orders) {
      ...
      if (target !== OrderStatus.PENDING) chain.push(OrderStatus.CONFIRMED);
      if ([OrderStatus.PREPARING, OrderStatus.READY, OrderStatus.DELIVERED].includes(target)) chain.push(OrderStatus.PREPARING);
      if ([OrderStatus.READY, OrderStatus.DELIVERED].includes(target)) chain.push(OrderStatus.READY);
      if (target === OrderStatus.DELIVERED) chain.push(OrderStatus.DELIVERED);
      for (const s of chain) await ordersService.updateStatus(order.id, store.id, s);
    }
